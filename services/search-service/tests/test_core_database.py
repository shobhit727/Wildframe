...
class TestInit:
    ...
    async def test_a_second_init_replaces_the_engine_without_disposing_it(
        ...
        The previously created engine is overwritten and never disposed, so a
        repeated init (as ``health_check`` performs when the engine is None)
        leaves the old pool unclosed. Reported, not fixed.
        """
...
class TestClose:
    ...
    @pytest.mark.asyncio
    async def test_close_keeps_the_factory_but_drops_the_engine(self, reset_manager):
        engine = _engine_mock()
        DatabaseManager.engine = engine
        DatabaseManager.session_factory = MagicMock()
...
class TestGetDb:
    @pytest.mark.asyncio
    async def test_yields_a_session_from_a_transaction_scope(self, reset_manager):
