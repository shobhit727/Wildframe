...
async def test_a_log_record_emitted_after_set_request_id_carries_the_id():
    """The contextvar only earns its keep if a handler can read it."""
    seen: list[str] = []
    handler = logging.Handler()
    handler.emit = lambda record: seen.append(request_id_var.get())  # type: ignore[method-assign]
    logging.getLogger("app.test").addHandler(handler)
    logging.getLogger("app.test").setLevel(logging.INFO)
    try:
        set_request_id()
        logging.getLogger("app.test").info("hello")
    finally:
        logging.getLogger("app.test").removeHandler(handler)
    assert seen and uuid.UUID(seen[0])
