...
class TestProxy:
    ...
    def test_proxy_preserves_method_and_headers(self, client):
        ...
        # X-Request-ID should be stripped and regenerated server-side (#522, #625)
        # but the test verifies the client's header was not forwarded as-is
        forwarded_headers = call_kwargs["headers"]
        # The client's X-Request-ID should be stripped; server generates its own
        assert "x-request-id" not in forwarded_headers or forwarded_headers.get("x-request-id") != "req-1"
    ...
    def test_shared_client_lifespan_creates_client_with_limits(self):
        ...
        async def run():
            async with shared_client_lifespan():
                ...
                assert client._limits.max_connections == settings.UPSTREAM_MAX_CONNECTIONS
                assert client._limits.max_keepalive_connections == settings.UPSTREAM_MAX_KEEPALIVE
                assert client._timeout.connect == settings.UPSTREAM_CONNECT_TIMEOUT
                assert client._timeout.read == settings.UPSTREAM_READ_TIMEOUT
                assert client._timeout.write == settings.UPSTREAM_WRITE_TIMEOUT
                assert client._timeout.pool == settings.UPSTREAM_POOL_TIMEOUT
