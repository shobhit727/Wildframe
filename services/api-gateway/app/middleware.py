...
class RateLimiter:
    ...
    def _get_limit(self, service: str, path: str = "") -> int:
        ...
        if service == "uploads" and (
            path.endswith("/complete") or path.endswith("/abort")
        ):
...
class HeaderSanitizerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        request._headers = None  # type: ignore[attr-defined]  # force re-parse
        request.scope = scope
...
#    - Service-level caps: multipart (uploads) vs JSON (default)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        body_limit = self.max_multipart_body if (is_multipart or service == "uploads") else self.max_request_body
        ...
        if request.headers.get("content-encoding", "").lower() in ("gzip", "deflate", "br"):
            ...
            if compressed_size > 0 and compressed_size * self.max_decompression_ratio > body_limit:
                return Response(
                    content=f"Potential decompression bomb: {compressed_size} * {self.max_decompression_ratio} > {body_limit}",
                    status_code=413,
                    headers={"X-Request-ID": request.headers.get("x-request-id", "")},
                )
        ...
        if isinstance(response, StreamingResponse):
            return self._stream_with_limit(response)
