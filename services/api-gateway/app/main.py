...
async def lifespan(app: FastAPI):
    ...
    global auth_middleware, rate_limiter, _shutdown_event, _in_flight_requests
    # Startup
    ...
    # Start shared AsyncClient lifespan
    ...
    _shutdown_event.set()  # signal no new requests
...
def create_app() -> FastAPI:
    ...
    # 1. BodyLimitMiddleware (innermost - enforces limits first)
    # 2. HeaderSanitizerMiddleware (strips/rewrites headers)
    # 3. AuthenticationMiddleware (checks auth on protected routes)
    # 4. RateLimiter (checks rate limits)
    # 5. CORSMiddleware
    # 6. Observability (added by wire_observability)
