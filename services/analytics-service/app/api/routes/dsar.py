...
async def export_analytics(user_id: UUID, format: str = Query(default="json", pattern="^(json|csv)$")) -> list[AnalyticsExportResponse]:
...
async def check_retention(user_id: UUID, retention_days: int) -> dict:
    ...
    return {"user_id": str(user_id), "retention_days": retention_days, "max_retention": max_retention, "compliant": compliant, "required_action": None if compliant else "reduce_retention_period"}
