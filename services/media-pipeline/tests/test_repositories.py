...
async def test_stage_log_record_and_list(db_session: AsyncSession):
    ...
    job = PipelineJob(
        id=uuid.uuid4(), upload_session_id=uuid.uuid4(), status=PipelineJobStatus.PENDING
    )
    ...
    log1 = PipelineStageLog(
        job_id=job.id, stage="ingest", status=PipelineStageStatus.SUCCESS, details={}
    )
    log2 = PipelineStageLog(
        job_id=job.id, stage="encode", status=PipelineStageStatus.FAILED, details={"error": "boom"}
    )
