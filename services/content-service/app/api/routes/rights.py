...
async def create_holder(
    request: RightsHolderCreate, db: Annotated[AsyncSession, Depends(get_db)]
) -> dict:
...
async def create_license(
    request: TerritorialLicenseCreate, db: Annotated[AsyncSession, Depends(get_db)]
) -> dict:
    from app.models.rights import TerritorialLicense

    lic = TerritorialLicense(
        content_id=request.content_id,
        rights_holder_id=request.rights_holder_id,
        territory=request.territory,
        exclusive=request.exclusive,
        avail_start=request.avail_start,
        avail_end=request.avail_end,
