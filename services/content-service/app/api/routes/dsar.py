...
async def export_content_dsar(
    user_id: UUID,
    dsar_id: UUID,
    db: Annotated[AsyncSession, Depends(get_db)],
) -> list[ContentDSARResponse]:
    """Export content-specific data for DSAR - viewing history, uploads, reviews."""
    from sqlalchemy import select
    from app.models.dsar import ContentDSARRecord

    stmt = select(ContentDSARRecord).where(
        ContentDSARRecord.user_id == user_id,
        ContentDSARRecord.dsar_id == dsar_id,
    )
