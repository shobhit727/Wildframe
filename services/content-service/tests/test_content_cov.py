...
def test_rights_exclusive():
    _ = RightsHolder(name="H", type="creator")
    license_obj = TerritorialLicense(
        content_id=uuid4(),
        rights_holder_id=uuid4(),
        territory="IN",
        exclusive=False,
        avail_start=datetime.now(UTC),
        avail_end=datetime.now(UTC),
    )
    assert license_obj.exclusive is False
    assert license_obj.territory == "IN"
