...
class StreamingService:
    ...
    async def update_playback_session(
        self, session_id: UUID, request: PlaybackSessionUpdateRequest
    ):
    ...
    async def check_session_valid_for_playback(self, session_id: UUID, user_id: UUID | None) -> bool:
        ...
        ``user_id`` may be None on the signed-URL path (#489, #491): the signature
        proves possession of the session, so only status/expiry are enforced.
        """
...
"""
    ...
    async def update_transcoding_progress(
        self, job_id: UUID, progress_percent: int, error_message: str | None = None
    ):
        """Update transcoding job progress."""
        try:
            job = await self.transcoding_repo.update(
                job_id, progress_percent=progress_percent, error_message=error_message
            )
            await self.transcoding_repo.commit()
            return job
        except Exception as e:
    ...
    async def get_quality_profiles_for_bandwidth(self, bandwidth_kbps: int):
        ...
        return [
            ...
            if p.min_bandwidth_kbps <= bandwidth_kbps <= p.max_bandwidth_kbps
        ]
    ...
    async def update_download_progress(self, download_id: UUID, bytes_downloaded: int):
        ...
        try:
            ...
            progress_percent = (
                int((bytes_downloaded / download.total_bytes) * 100)
                if download.total_bytes > 0
                else 0
            )
            download = await self.download_repo.update(
                download_id, bytes_downloaded=bytes_downloaded, progress_percent=progress_percent
            )
            await self.download_repo.commit()
            return download
        except Exception as e:
